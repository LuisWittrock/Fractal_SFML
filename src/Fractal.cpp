...
std::pair<double, double> Fractal::screenToWorld(int pixelCoordReal, int pixelCoordImg) //translates pixel coordinate to a coordinate of a complex number (a*bi)
{
    ...
    double ImgCoord = (double)(pixelCoordImg * ((graphYMax - graphYMin) / (double)WINDOW_HEIGHT) + ((graphYMax - graphYMin)/WINDOW_HEIGHT/2)) * scale + graphYMin;
    double RealCoord = (double)(pixelCoordReal * ((graphXMax - graphXMin) / (double)WINDOW_WIDTH) + ((graphXMax - graphXMin)/WINDOW_WIDTH/2)) * scale + graphXMin;
...
void Fractal::colorMixer(int escape, sf::Vertex &vertex)
{
    ...
    if (escape == bailout)
        ...
        vertex.color = Color(rgb[escape % 16][0], rgb[escape % 16][1], rgb[escape % 16][2]);
}
